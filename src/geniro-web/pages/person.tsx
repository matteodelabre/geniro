...
const renderProjects = (projects) =>
    renderTable(projects, {
        ...
        title: {
            label: "Titre",
            display: (title, row) => <a href={uriToUrl(row.preferredUri)}>{title}</a>,
        },
        dateEnd: {
            label: "Date",
...
const renderPerson = (personData, advisedProjectsData, studentProjectsData) => (
    <>
        ...
        {renderProjects(Object.values(advisedProjectsData))}

        <h3>Projets réalisés</h3>
        {renderProjects(Object.values(studentProjectsData))}
    </>
);
...
person.get("/:id", async (ctx) => {
    ...
    render(ctx, {
        ...
        content: renderPerson(personData, advisedProjectsData, studentProjectsData),
    });
});
